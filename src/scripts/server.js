...
const p12Password = process.env.PRIVATE_KEY_PASSWORD.trim();
console.log('Private key password: ', p12Password);


const readPkcs12Async = (p12Path, p12Password) => {
    return new Promise((resolve, reject) => {
        pem.readPkcs12(readFileSync(p12Path), {p12Password}, (err, cert) => {
            if (err) {
                reject('Error reading p12 file: ' + err);
            } else {
                resolve(cert);
            }
        });
...
// Se puede emplear el mismo puerto que HTTP, no lo intentes cambiar para arreglarlo *palm face*
        httpsServer.listen(8080, () => console.log("LISTENING ON 8080"));
...
// Crear sala
            socket.on('create', (nombreHost, juego) => {
                ...
                // Aquí es donde realmente se conecta al lobby
